...
export default async function datepickerSelect(selector, selected) {
  assert('`datepickerSelect` expect a date string as second argument', (typeof selected === 'string'));
  let selectorElement = find(selector);
  assert(`'datepickerSelect' couldn't find any element with selector: ${selector}`, !!selectorElement);
  let triggerElement;
  ...
  return calendarSelect('.ember-power-datepicker-content', moment(selected).utc().toDate());
}
